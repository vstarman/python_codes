...
def judge_online_add(user_name, host_name, goal_ip):
    """判断用户是否在列表中，没有则添加进列表"""
    for info in feiq_constant.user_list:
        ...
        online_user = {}
        online_user["ip"] == goal_ip
        online_user["user_name"] == user_name
        online_user["host_name"] == host_name
...
def recv_msg():
    while 1:
        ...
        if command == feiq_constant.IPMSG_BR_ENTRY:
            ...
            # 添加用户到列表
            find_post = data_dict["massage"].find("\0")
            if find_post != -1:
                user_name = data_dict["massage"][:find_post]
            else:
                user_name = data_dict["massage"]
            judge_online_add(user_name, data_dict["host_name"], goal_addr[0])
            # 通报对方，我已在线
            answer_msg = feiq_send.build_msg(feiq_constant.IPMSG_ANSENTRY)
            feiq_send.build_send(answer_msg, goal_addr[0])
        ...
        if command == feiq_constant.IPMSG_ANSENTRY:
            # 用户已在线
            print("%s已在线" % data_dict["massage"])
            # 查询是否在列表中
            judge_online_add(data_dict["massage"][:data_dict["massage"].find("\0")], data_dict["host_name"], goal_addr[0])
        ...
        if command == feiq_constant.IPMSG_SENDMSG:
            ...
            # 给对方发送消息确认
            recv_back_msg = feiq_send.build_msg(feiq_constant.IPMSG_RECVMSG)
            feiq_send.build_send(recv_back_msg, (goal_addr[0], feiq_constant.feiq_port))
