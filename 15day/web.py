...
class HTTPServer(object):
    ...
    def client_handler(self, client_socket):
        ...
        # handle dynamic request(处理动态请求)
        if path_info.endswith(".py"):
            # response body is return original data(返回原始数据)
            # first parameter is type dict environ(环境字典变量)
            # second parameter is module quote method(模块方法的引用)
            response_body = self.app(environ, self.start_response)

            # WSGI suggest(建议) response body type should be bite
